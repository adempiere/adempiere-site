...
- Reference: Data Type of the column- each reference corresponds to a different behavior in the GUI. Please note carefully the difference between Table and Table Direct. Table Direct needs an exact match of the case for the table with the name of the column (excepting the suffix _ID)
...
- Read-only logic – condition for making the record read-only (by default IsActive and Processed columns mark the record as readonly without the need to define the logic here)
...
- Callout – piece of code (customization) for filling other fields or simple validations (not recommended for validations – you still need to validate on saving) – for string fields, the callout is called on a keystroke basis
...
- Key Column – this is the key column of the table, this value is stored in database – normally is the key (table_ID)
...
Also see [ManPageW_ValidationRules](https://wiki.adempiere.net/ManPageW_ValidationRules) for more details on the validation rule window.
...
- Create menu tree
- Add (existing) nodes in window Tree Maintenance
- Assign the new menu in Role

The menu terminology is synchronized with windows and processes names.

**_Exercise:_**

- Create a view to get product category value, name, and the count of products in such a product category.
